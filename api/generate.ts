...
Examples of ${maxLetters}-letter words or shorter: ${getExampleWords(maxLetters)}
...
Write only the story text, nothing else. No titles, no explanations, just the story.`;
    ...
    if (longWords.length > 0) {
      ...
      const retryResult = await model.generateContent(prompt + '\n\nREMINDER: Every single word MUST be ' + maxLetters + ' letters or fewer!');
...
function getExampleWords(maxLetters: number): string {
  const examples: Record<number, string[]> = {
    ...
    4: ['play', 'tree', 'bird', 'jump', 'rain', 'bear', 'fish', 'love', 'soft', 'warm'],
    5: ['happy', 'smile', 'dance', 'green', 'quick', 'sweet', 'small', 'cloud', 'dream', 'shine'],
    6: ['forest', 'rabbit', 'gentle', 'purple', 'splash', 'friend', 'bright', 'breeze', 'nature', 'giggle'],
    7: ['rainbow', 'magical', 'sparkle', 'whisper', 'curious', 'flutter', 'puddles', 'sunbeam', 'morning', 'journey'],
    8: ['sunshine', 'treasure', 'elephant', 'laughter', 'discover', 'colorful', 'friendly', 'grateful', 'peaceful', 'adorable'],
  };
